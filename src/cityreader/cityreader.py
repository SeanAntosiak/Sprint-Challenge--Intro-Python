...
def cityreader(cities=[]):
    with open('cities.csv') as cities_csv:
        rows = csv.reader(cities_csv)
        for row in rows:
            cities.append(row)
        cities.remove(cities[0])
    records = []
    for c in cities:
        if ((c[0] != 'name') & (c[3] != 'lat') & (c[4] != 'lon')):  # there was a header in the middle of the file
            records.append(City(f"{c[0]}", float(c[3]), float(c[4])))
...
for i in range(60):
    if check_city(cities[i], correct_cities[i]) is False:
        print(f'{i} was not equal')
